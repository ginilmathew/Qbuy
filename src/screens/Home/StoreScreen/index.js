...
const StoreScreen = ({ route }) => {
    ...
    return (
        <>
            <HeaderWithTitle title={ mode === 'store' ? item?.store_name?.length > 18 ? item?.store_name?.slice(0, 18) + '....' : item?.store_name : name?.length > 18 ? name?.slice(0, 18) + '....' : name } onPressBack={ mode === 'cartItem' ? backToCart : mode === 'checkoutItem' ? backToCheckout : '' } />
            <ScrollView
